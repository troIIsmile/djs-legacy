...
export const run = async (message: Message, args: string[]) => {
  ...
  const { Id: id } = await fetch('https://api.roblox.com/users/get-by-username?username=' + encodeURIComponent(args.join(' '))).then(res=>res.json())
    if (!id) {
    return {
      embed: {
        author: {
  ...
  const friendarray = await fetch(`https://api.roblox.com/users/${id}/friends`).then(res=>res.json())
  if (!friendarray) {
    return {
      embed: {
        author: {
  ...
  const fields = friendarray.map((friend: RobloxFriend)=>{return {
